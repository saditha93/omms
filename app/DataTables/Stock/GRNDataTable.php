...
class GRNDataTable extends DataTable
{
        ...
        return datatables()
            ...
            ->addColumn('action', function ($item) {
                if ($item->active == 2) {
                    ...
                    return '<div class="w-80"></div><a href="' . route('gRNHeader.show', $item->id) . '" class="btn btn-xs btn-warning" data-toggle="tooltip" data-placement="bottom" title="Good Recive Note Details"><i class="fa fa-eye"></i></a>
                        <a href="' . route('gRNDetail.create', 'id=' . $item->id) . '" class="btn btn-xs btn-success" data-toggle="tooltip" data-placement="bottom" title="Add Items to Good Recive Note"><i class="fa fa-check-square"></i></a>
    ...
    {
        return $model->newQuery()->with('establishment', 'supplier')->where('establishment_id', Auth::user()->mess_id)->where('is_bar', 0);;
    }
