            ...
            <div class="card-body">
                @if(!isset($item->id))
                    <form method="POST" action="" id="orderForm">
                        ...
                        <div class="col-md-12 text-right mt-5"><button class="btn btn-dark" id="btn_pdf">Generate PDF</button></div>
...
@push('scripts')    
    ...
    <script src="{{asset('/js/thisPDF/printThis.js')}}"></script>
        ...
        $(document).ready(function () {
            ...
            $("#order_report_search").click(function() {
                ...
                if (officer_enum == "") {
                    ...
                    $.ajax({
                        url:"{{ url('') }}/admin/all_order_details",
                        method:'POST',
            ...
            $("#btn_pdf").click(function() {
                ...
                if ($('#officer').val() && $('#datepicker1').val() && $('#datepicker2').val())
                {
                    $filename = $( "#officer option:selected" ).text()+' from_'+$('#datepicker1').val()+'_to_'+$('#datepicker2').val();

                    document.title = window.parent.document.title = $filename;
                    $('#mess_bill_report_div').printThis({
                        header: `<h1 class="text-center">Mess Bill of `+$( "#officer option:selected" ).text()+`<br> <p>From `+$('#datepicker1').val()+` To`+$('#datepicker2').val()+`</h1>`,
                    });

                }
                else
                {
                    Swal.fire('Please select the officer name and relevant dates!', '', 'info');


                }

                // document.title = window.parent.document.title = "OMMS";
...
@push('page_css')
    {{--pdf--}}
{{--    <link rel="stylesheet" href="{{asset('/css/thisPDF/normalize.css')}}"/>--}}
{{--    <link rel="stylesheet" href="{{asset('/css/thisPDF/skeleton.css')}}"/>--}}
@endpush
