...
@section('content')
    ...
    <div class="col-md-12 pt-2">
        <div class="card">
            ...
            <div class="card-body">
                @if(!isset($item->id))
                    <form method="POST" action="{{route('dessert-items-price-set')}}">
                        @else
                    <form method="POST" action="{{route('dessert-item-update',$item->id)}}">
                                @method('PUT')
                                @endif
                                @csrf
                                <div class="row">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select class="form-control" name="item" id="item" {{isset($item->item_id)?'disabled':''}}>
                                                ...
                                                @foreach($dessertItems as $dessertItem)
                                                    <option value="{{$dessertItem->id}}"  {{isset($item->item_id)?$item->item_id==$dessertItem->id?'selected':'':''}}>{{$dessertItem->item_name}}</option>
                                                @endforeach
                                            </select>
                                            @error('item')
                                    ...
                                    <div class="col-md-12">
                                        <div class="form-group">
                                            @if(!isset($item->id))
                                                <input class="btn btn-primary float-right" type="submit" value="Add Dessert Item Price">
                                            @else
                                                <input class="btn btn-primary float-right" type="submit" value="Update Dessert Item Price">
                                                <a class="btn btn-secondary float-right mr-1" type="button" href="{{route('dessert')}}">Cancel</a>
                                            @endif
                                        </div>
                                    </div>
                                </div>
    ...
    <div class="col-md-12">
        <div class="card">
            <div class="card-body">
                {{--{{ $dataTable->table() }}--}}
                <table id="allDessertTbl" class="display" style="width:100%">
                    <thead>
                    <tr>
                        <th>Index</th>
                        <th>Item Name</th>
...
<script src="{{asset('/js/datepicker/datepicker.js')}}"></script>
    ...
    <script>
        ...
        //datatable
        function dessertDatatable()
        {
            //dtaTable
            $('#allDessertTbl').DataTable({
                // responsive: true,
                // processing: true,
                serverSide: true,
                ajax: {
                    headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                    url: '{{url("admin/get-dessert-items")}}',
                    type: "POST",
                    dataType: "json",
                },
                columns: [
...
@push('page_css')
    <style>
        #allDessertTbl_wrapper .dt-buttons{
