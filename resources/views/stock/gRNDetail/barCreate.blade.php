    ...
    <div class="container-fluid">
        ...
        <section class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <h1>Add Bar Items to GRN {{str_pad($header_id, 10, "0", STR_PAD_LEFT)}}</h1>
                    </div>
                    <div class="col-sm-6">
                            ...
                            <div class="col-sm-9">
                                <select required required name="item_id" id="item_id"
                                    ...
                                    @foreach($items as $item)
                                        <option
                                            @selected($item->id == old('item_id')) value="{{$item->id}}">{{$item->name}}
                                            - {{$item->measure_unit->abbreviation}}
                                            ({{$item->measure_unit->size}} {{$item->measure_unit->size_type}})
                                        </option>
                                    @endforeach
                                </select>
                                @error('item_id')
    ...
    <script>
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (confirm("Are you sure you want to close?")) {
                        window.location.href = "{{ route('gRNHeader.bar.index')}}";
                    }
                    return false;
                }
            });
        ...
        $('#item_id').change(function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                    $('#measure_unit').val(data.measure_unit.abbreviation + ' (' + data.measure_unit.size + ' ' + data.measure_unit.size_type + ')');
                }
            });
        })
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    $.each(response, function (key, value) {
                        ...
                        $('#item_id').append(new Option(val, value.id));
                    });
                }
            });
