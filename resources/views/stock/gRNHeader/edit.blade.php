    ...
    <div class="container-fluid">
        ...
        <section class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <h1>Edit Good Receive Note {{ $gRNHeader->order_no }}</h1>
                    </div>
                    <div class="col-sm-6">
        ...
        <div>
            ...
            <div class="card">
                <div class="card-header">
                    <div class="card-title">Edit Good Receive Note </div>
                ...
                <form role="form" method="POST" action="{{ route('gRNHeader.update',$gRNHeader->id)  }}"
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-9">
                                <input type="text" name="no"
                                       ...
                                       placeholder="Bill No" value="{{ $gRNHeader->no}}">
                                @error('no')
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-9">
                                    <select required required name="establishment_id" id="establishment_id"
                                        ...
                                        @if(isset($gRNHeader->establishment->id))
                                            @foreach($establisments as $establisment)
                                                <option
                                                    @selected($gRNHeader->establishment->id == $establisment->id)  value="{{$establisment->id}}">{{$establisment->name}}</option>
                                            @endforeach
                                        @else
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-9">
                                <select required name="supplier_id" id="supplier_id"
                                        class="bg-gray-50 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                                    @if(isset($gRNHeader->supplier->id))
                                        @foreach($suppliers as $supplier)
                                            <option
                                                @selected($gRNHeader->supplier->id == $supplier->id)  value="{{$supplier->id}}">{{$supplier->name}}</option>
                                        @endforeach
                                    @else
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-9">
                                <textarea type="text" name="remarks"
                                          ...
                                          id="adjusted_stock"
                                          placeholder="Remark">{{$gRNHeader->remarks}} </textarea>
                                @error('remarks')
                                <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
...
@section('third_party_scripts')
    ...
    <script>
        ...
        const picker = MCDatepicker.create({
            ...
            closeOnBlur: true,
            maxDate: new Date(),
            selectedDate: new Date(selectDate[0], selectDate[1], selectDate[2]),
            theme: {
                theme_color: '#151414',
